...
static intptr_t augmenting_path(intptr_t nc, double *cost, std::vector<double> &u,
...
static int solve(intptr_t nr, intptr_t nc, double *cost, bool maximize, int64_t *a,
                 int64_t *b) {
    ...
    for (intptr_t curRow = 0; curRow < nr; curRow++) {
        ...
        double minVal;
        intptr_t sink =
            augmenting_path(nc, cost, u, v, path, row4col, shortestPathCosts, curRow,
                            SR, SC, remaining, &minVal);
        if (sink < 0) {
