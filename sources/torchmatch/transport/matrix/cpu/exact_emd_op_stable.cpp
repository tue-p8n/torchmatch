...
Tensor exact_emd_kernel_cpu(const Tensor &cost, const std::optional<Tensor> &mask,
                            const std::optional<Tensor> &a,
                            const std::optional<Tensor> &b) {
    STD_TORCH_CHECK(cost.dim() == 3,
                    "torch.ops.transport.exact_emd: cost.ndim must be 3, got ",
                    cost.dim());
    STD_TORCH_CHECK(cost.is_cpu(), "torch.ops.transport.exact_emd: cost must be CPU");
    STD_TORCH_CHECK(
        cost.scalar_type() == ScT::Float || cost.scalar_type() == ScT::Double,
    ...
    if (mask.has_value()) {
        ...
        STD_TORCH_CHECK(
            mask->dim() == 3 && mask->size(0) == B && mask->size(1) == N &&
                mask->size(2) == M,
    ...
    ts::parallel_for(0, B, 1, [&](int64_t begin, int64_t end) {
        for (int64_t bi = begin; bi < end; ++bi) {
            ...
            int status = EMD_wrap(
                ...
                a_b.mutable_data_ptr<double>(), b_b.mutable_data_ptr<double>(),
                cost_b.mutable_data_ptr<double>(), plan_d.mutable_data_ptr<double>(),
                alpha.mutable_data_ptr<double>(), beta.mutable_data_ptr<double>(),
