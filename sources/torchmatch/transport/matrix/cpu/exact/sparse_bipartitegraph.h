...
class SparseBipartiteDigraphBase {
  ...
  protected:
    ...
    mutable std::vector<int64_t>
        _arc_to_out_pos; // _arc_to_out_pos[arc_id] = position in _arc_ids
    mutable std::vector<int64_t>
        _arc_to_in_pos; // _arc_to_in_pos[arc_id] = position in _in_arcs[target]
    mutable bool _position_maps_built;

    SparseBipartiteDigraphBase()
        : _node_num(0), _arc_num(0), _n1(0), _n2(0), _in_arcs_built(false),
