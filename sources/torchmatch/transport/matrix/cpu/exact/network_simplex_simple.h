...
// if your compiler has troubles with stdext or hashmaps, just comment the following
// line to use a slower std::map instead
// #define HASHMAP
...
class SparseValueVector {
  public:
    ...
    ProxyObject<T> operator[](const size_t id) { return ProxyObject<T>(this, id); }
...
class ProxyObject {
  public:
    ProxyObject(SparseValueVector<T> *v, size_t idx) {
        _v = v;
        _idx = idx;
    };
...
/// flow problem.
///
/// %NetworkSimplexSimple is the dense network-simplex implementation
/// used by this directory's transport solver.
...
class NetworkSimplexSimple {
  public:
    ...
    NetworkSimplexSimple(const GR &graph, bool arc_mixing, int nbnodes,
        ...
        : _graph(graph), //_arc_id(graph),
          ...
          INF(std::numeric_limits<Value>::has_infinity
                  ...
                  : MAX),
          _lazy_cost(false), _coords_a(nullptr), _coords_b(nullptr), _dim(0),
          _metric(0), _n1(0), _n2(0), _dense_cost(false), _D_ptr(nullptr), _D_n2(0),
          _warmstart_provided(false), _warmstart_tree_built(false), _max_cost(0),
          _has_max_cost(false) {
      ...
      public:
        ...
        BlockSearchPivotRule(NetworkSimplexSimple &ns)
            : _source(ns._source), _target(ns._target), _cost(ns._cost),
              ...
              _search_arc_num(ns._search_arc_num), _next_arc(0), _ns(ns) {
        ...
        bool findEnteringArc() {
            ...
            for (e = _next_arc; e != _search_arc_num; ++e) {
                ...
                if (--cnt == 0) {
                    a = fabs(_pi[_source[_in_arc]]) > fabs(_pi[_target[_in_arc]])
                            ? fabs(_pi[_source[_in_arc]])
                            : fabs(_pi[_target[_in_arc]]);
  ...
  public:
    ...
    inline Cost getCostForArc(ArcsType arc_id) const {
        if (_dense_cost) {
            ...
            // Without arc mixing: internal arc_id maps to graph arc = _arc_num - arc_id
            // - 1 graph arc g encodes source i = g / m, target j = g % m cost = D[i *
            // _D_n2 + j] = D[g] (since m == _D_n2)
            ...
            // Convert internal node IDs back to graph node IDs, then to coordinate
            // indices
            int i = _node_num - _source[arc_id] - 1; // graph source in [0, _n1-1]
            int j = _node_num - _target[arc_id] - 1 -
                    _n1; // graph target in [_n1, _node_num-1] -> [0, _n2-1]
    ...
    void setWarmstartPotentials(const Cost *alpha, const Cost *beta, int n, int m) {
        ...
        // Note: warmstartInit() will refine these by recomputing from the tree
        // structure.
    ...
    int64_t divid(int64_t x, int64_t y) { return (x - x % y) / y; }
    ...
    ///
    /// \pre \ref run() must be called before using this function.
    Value flow(const Arc &a) const { return _flow[getArcID(a)]; }
  ...
  private:
    ...
    bool warmstartInit() {
...

        {
            ...
            for (ArcsType idx = 0;
                 idx < (ArcsType)candidates.size() && tree_edges < _node_num - 1;
                 ++idx) {
    ...
    bool findLeavingArc() {
        ...
        // Search the cycle along the path from the second node to the root
    ...
    void updateTreeStructure() {
        ...
        for (u = v_in; u != up_limit_in && _last_succ[u] == v_in; u = _parent[u]) {
    ...
    ProblemType start() {
        ...
        if (retVal == OPTIMAL) {
            for (ArcsType e = _search_arc_num; e != _all_arc_num; ++e) {
                if (_flow[e] != 0) {
                    if (fabs(_flow[e]) >
                        _EPSILON) // change of the original code following issue #126
                        return INFEASIBLE;
                    else
                        _flow[e] = 0;
                }
