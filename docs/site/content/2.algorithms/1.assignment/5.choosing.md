...
| `torch.compile(mode="reduce-overhead")` (CUDA graphs) | None of the CUDA-only assignment ops. `munkres`, `hybrid`, `lawler`, and the CUDA backend of `jonker_dense_batch` all synchronize with the CPU during execution and cannot be captured into a CUDA graph — each raises a clear error if called inside capture. Call them outside the compiled region. |
