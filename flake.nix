  ...
  outputs = inputs @ {
    ...
    flake-parts.lib.mkFlake {inherit inputs;} {
      ...
      perSystem = {
        ...
        in
          if lib.hasPrefix "13.2" v
          then variants.cu132
          else if lib.hasPrefix "13." v
          then variants.cu130
          else if v == "12.6"
          then variants.cu126
          else if lib.hasPrefix "12." v
          then variants.cu128
          else throw "torchmatchWithCuda: no wheel for CUDA ${v}. Supported: 12.6, 12.8, 13.x";
      in {
      ...
      flake.overlays.default = final: _prev: let
        ...
        cudaVariant =
          if cudaVersion == null
          then throw "torchmatch overlay: pkgs.cudaPackages has no cudaMajorMinorVersion"
          else if lib.hasPrefix "13.2" cudaVersion
          then variantsFromFinal.cu132
          else if lib.hasPrefix "13." cudaVersion
          then variantsFromFinal.cu130
          else if cudaVersion == "12.6"
          then variantsFromFinal.cu126
          else if lib.hasPrefix "12." cudaVersion
          then variantsFromFinal.cu128
          else throw "torchmatch overlay: no wheel for CUDA ${cudaVersion}. Supported: 12.6, 12.8, 13.x";
      in {
